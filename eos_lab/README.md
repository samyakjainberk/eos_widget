...
- **`prediction.py`** — the **`/prediction`-widget forecasts** ported from `server.py`: the `(lr, init-std)` **`sweep()`** (predictions 1 / 2 / 2b, incl. the start-of-training `‖J‖`-trend `jnormSign`), **`pred_signchange()`** (cubic Eq-51 forecast of the `‖J·ṙ‖−‖J̇·r‖` sign-change), and three per-step trackers: **`Pred3Tracker`** (frozen-`J` residual theory, 1st + 2nd order, + the `t*−10 / +10 / +20` freeze sweep), **`Pred4Tracker`** (frozen- vs **evolving**-residual NTK-eigenvalue propagation + NTK/Gauss-Newton eigenvector cosines + the `t₀−10 / +10 / +20` freeze sweep), and **`TraceTracker`** (`Tr(NTK)` quad/cubic × live/self forecasts vs `Tr(∇²L)`/`Tr(JᵀJ)`). Exact-autograd: the pure-linear / NTK-eigenvalue pieces match the server to machine precision (≈1e-9…1e-16), and the `hvp_S`/`hvp_L`-routed quantities (Pred-4's `M_r·J` propagation, Trace's `trHess`) carry the documented **~1e-3** FD-vs-exact gap (parity test worst-field error 4.5e-5, all within tolerance — verified against the live server).
