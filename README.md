...
Sharpness `σ₁ = λ_max(∇²_θ L)` rises (*progressive sharpening*) until it pins near `2/η` at the edge of
stability, where the loss oscillates with period 2. Over a local window the model is replaced by its
frozen second-order Taylor surrogate about `θ_t`:

```
f_{θ_{t+Δt}} = f_{θ_t} + J_{θ_t}ᵀ Δθ + Δθᵀ Q_{θ_t} Δθ ,   Q_{θ_{t+Δt}} = Q_{θ_t}    (Eq. 1)
```

with Jacobian `J = df/dθ` and function Hessian `Q = d²f/dθ²`. Predictions assume (1) Gauss-Newton `G` sets
the Hessian's top eigenvalues, (2) the surrogate holds for a few steps, (3) the residual `r = y−f` (or its
top-NTK projection) keeps its sign. For MSE, `∇²L = G + S` with `G = (1/N)Σ ∇fᵢ∇fᵢᵀ` and
`S = (1/N)Σ (fᵢ−yᵢ)∇²fᵢ`. The §9 panel forward-predicts `σ₁` from Eqs. 13/21/27/29 and overlays the measured value.
